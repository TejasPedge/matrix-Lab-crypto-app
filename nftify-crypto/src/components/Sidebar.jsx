...
import { FacebookIcon, TwitterIcon, LinkedinIcon } from './Social Icon SVGs/SocialIcons'
import { NavLink } from 'react-router-dom'
import { useSelector } from 'react-redux'
import {AiFillCloseCircle} from 'react-icons/ai'
import { useDispatch } from 'react-redux'
import { hideSidebar } from '../Redux/pairTokenSlice'
import { initializeCryptoData, initializePairData } from '../Redux/pairTokenSlice'

const Sidebar = () => {

    const {isSideBarHidden} = useSelector(({pairTokenReducer}) => pairTokenReducer);

    const dispatch = useDispatch();

    console.log('hidden',isSideBarHidden);
  ...
  return (
    ...
    >
        {/* close icon */}
        <Box display = {{base : "block", '639px' : 'none'}} onClick = {handleHideSidebar} position = 'absolute' right = '10px' top = '20px' boxShadow = {'2px 2px 10px black'} borderRadius = {'50%'} overflow = {'hidden'}>
        ...
        <Box display={'flex'} flexDirection = {'column'} justifyContent = {"space-between"} height = '100%'> 
            <Box display={'flex'} flexDirection = {'column'}>
                <Box p = {'16px 0 17px 35px'} display={'flex'} alignItems = {'center'}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="25.33333" height="29.33333" viewBox="0 0 26 30" fill="none">
                    ...
                    <Text marginLeft = {'24.67px'} color = "rgba(255, 255, 255, 0.70)" fontFamily={'Pacifico'} fontSize = "32px" fontStyle = "normal" fontWeight = "400" lineHeight = "normal">NFTify</Text>
                </Box>
                {/* ----------------- 2 ------------------ */}
                <NavLink onClick = {() => dispatch(initializeCryptoData())} style = {({isActive}) => ({background : isActive ? '#F30050' : "", marginBottom : "15px"})} to = '/'>
                    <Box p = {'17px 20px 17px 32px'} display={'flex'} alignItems = {'center'}>
                        ...
                        <Text color = "#F7F9F9" marginLeft = {'20px'} fontFamily={'Work Sans'} fontSize = "20px" fontStyle = "normal" fontWeight = "600" lineHeight = "28px">Token Address</Text>
                    </Box>
                </NavLink>
                {/* ----------------- 3 --------------------- */}
                <NavLink onClick = {() => dispatch(initializePairData())} style = {({isActive}) => ({background : isActive ? '#F30050' : ""})} to = '/Pair-Address'>
                    <Box p = {'17px 20px 17px 32px'} display={'flex'} alignItems = {'center'}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        ...
                        <Text color = "#F7F9F9" marginLeft = {'20px'} fontFamily={'Work Sans'} fontSize = "20px" fontStyle = "normal" fontWeight = "400" lineHeight = "28px">Pair Address</Text>
                    </Box>
                </NavLink>
            ...
            <Box ml = {'34px'} mb = {'125px'} display = {'flex'} justifyContent = {'space-between'} width = {'112px'}>
                <FacebookIcon />
                <LinkedinIcon />
                <TwitterIcon />
            </Box>
